...
export default async function SeoLandingPage({
  ...
  return (
    <div className="space-y-8">
      ...
      {pickupStructuredData?.map((item, index) => (
        <script
          key={index}
          type="application/ld+json"
      ...
      <section className="space-y-4">
        ...
        {catalog.products.length > 0 ? (
          ...
          <div className="card text-center text-muted">
            <p>No active products are visible on this page yet. Use the live catalog to browse the full archive.</p>
          </div>
        )}
      </section>
      ...
      {page.kind === "pickup" && (
        <section className="grid gap-4 lg:grid-cols-3">
          {page.faqs.map((faq) => (
            <div key={faq.question} className="rounded-[1.1rem] border border-border bg-white p-4 shadow-card">
